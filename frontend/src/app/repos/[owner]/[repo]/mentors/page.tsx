...
import { fetchMentors, saveMentors } from "@/lib/api";
import type { GitHubAccount } from "@/lib/types";
import { ToastContainer, useToast } from "@/components/toast";

export default function MentorsPage() {
  const params = useParams<{ owner: string; repo: string }>();
  const [mentors, setMentors] = useState<GitHubAccount[] | null>(null);
  ...
  useEffect(() => {
    fetchMentors(params.owner, params.repo)
      .then(setMentors)
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await saveMentors(params.owner, params.repo, mentors);
      showToast("Mentor roster saved successfully", "success");
    } catch (err) {
      showToast(
  ...
  return (
    <div>
      ...
      {mentors.length === 0 ? (
        <p className="text-oe-mid-gray mb-4">No mentors configured</p>
      ) : (
          ...
          <tbody>
            {mentors.map((mentor, i) => (
              <tr key={i} className="border-b border-oe-light-gray/50">
                <td className="py-2 text-oe-dark">{mentor.username}</td>
                <td className="py-2 text-oe-mid-gray">{mentor.githubId}</td>
